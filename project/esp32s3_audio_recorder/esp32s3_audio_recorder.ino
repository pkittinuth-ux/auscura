...
void startRecording() {
  ...
  Serial.println("\n[RECORDING & STREAMING] Started — 20 seconds...");
}
...
void doRecording() {
  ...
  // ครบ 20 วิ → จบการอัด
  if (elapsed >= (unsigned long)(RECORD_SECONDS * 1000)) {
    // ส่งข้อมูลที่เหลือใน Buffer ก๊อกสุดท้าย
    if (sendBufIdx > 0) {
  ...
  if (millis() - lastPrint >= 1000) {
    lastPrint = millis();
    Serial.printf("  [REC & STREAM] %lu sec / %d sec\n", elapsed / 1000, RECORD_SECONDS);
  }
}
...
void finishRecording() {
  ...
  currentState = IDLE;
  Serial.println("\nSystem Ready — Waiting for IR trigger...");
}
...
void setup() {
  ...
  pinMode(IR_PIN, INPUT_PULLUP);  // ใช้ PULLUP เผื่อไว้ในกรณีที่เซ็นเซอร์เป็นแบบ Open-Drain
...
void loop() {
  ...
  switch (currentState) {
    ...
    case IDLE:
      if (wsConnected && digitalRead(IR_PIN) == LOW) {
        delay(50);  // debounce
